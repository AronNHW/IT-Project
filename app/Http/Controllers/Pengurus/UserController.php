...
class UserController extends Controller
{
    ...
    public function update(Request $request, User $user)
    {
        ...
        if ($validator->fails()) {
            return redirect()->route('pengurus.users.index')->withErrors($validator)->withInput();
        }

        $user->update($request->only('name', 'email', 'no_wa', 'role'));

        return redirect()->route('pengurus.users.index')->with('success', 'User berhasil diperbarui.');
    }
